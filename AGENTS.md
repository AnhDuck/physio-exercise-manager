...
Do not run shell-launched headless Chrome/Edge, Playwright, Puppeteer, or Node browser automation here; local headless/GPU support is unreliable. Prefer the Codex in-app browser for local UI verification when it is available. If the in-app browser page opens for the user but automation fails with a tool/runtime error such as `node_repl kernel exited unexpectedly` or `windows sandbox failed: spawn setup refresh`, treat that as an automation bridge failure, not an app/server failure: stop retrying the same bridge, say this explicitly, and use a visible-browser/manual verification path against the same URL/origin. For local UI checks, cover compact grid, notes, settings, set tracker, backup warnings, image modal entry points, and ActivityWatch dashboard interactions when ActivityWatch changed. Always report whether browser verification was in-app automated, visible-browser/CDP based, manual, or skipped.

When a visible Chrome/Edge fallback is required, use the exact origin under test and avoid touching `8891`. Prefer an already-open tab on `http://127.0.0.1:<port>/index.html?v=<PEM_APP_VERSION>` when possible. If launching a browser with `Start-Process`, every argument must be a separate complete `-ArgumentList` item and any path value must stay attached to its switch, e.g. `--user-data-dir=C:\...\profile` as one item; paths under `C:\Users\Bill\Desktop\VIBE CODING\...` contain spaces, and bad quoting opens bogus tabs such as `file:///C:/Users/Bill/Desktop/VIBE` or `http://coding/...`. Do not pass workspace paths as loose positional arguments. Use a temporary profile path only for verification, and close the temporary browser after the check.
