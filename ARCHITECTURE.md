...
- `weather-card.js`: Open-Meteo weather card, current details including humidity/wind/UV/sun times, daily-brain advisory/highlight/mood rules, dev preview scenarios, location search, cached weather result, request throttling/rate-limit cooldowns, and weather Settings controls.
...
Dashboard card preferences, weather request cooldown metadata, and the cached last weather result live inside `pem_settings.homeCards` so backups and imports continue to use the existing settings safe-save path.
