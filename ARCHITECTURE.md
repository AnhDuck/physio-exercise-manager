...
- `weather-card.js`: Weather card DOM rendering only: full and collapsed summary states, setup/loading states, current details including AQHI-or-humidity/wind/UV/sun times, hourly strip, actions, advisory rendering, and the Sources disclosure. For Canadian locations, Environment Canada is the primary weather and AQHI source, while current UV prefers Open-Meteo and falls back to Environment Canada hourly UV only when Open-Meteo has no usable value. The compact Sources disclosure lists only available providers; fallback diagnostics and AQHI matching implementation details do not belong in the card. AQHI brain advisories start at moderate AQHI levels (`4+`) and include timing when the peak is later; lower AQHI values stay as quiet tile data.
