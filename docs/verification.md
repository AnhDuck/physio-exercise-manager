...
It covers personal-day boundaries and local-offset formatting, Canadian UV source precedence, auto-backup time/history/health policy, legacy migration transforms and idempotence, and first/middle/final-write storage rollback. Browser checks remain necessary for startup, settings, folder handles, and UI behavior.
