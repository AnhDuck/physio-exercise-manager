...
- Load answers how much total tendon load happened. Daily Load uses one simple bar per day for `Total tendon load`; Weekly Load groups Monday-Sunday and shows average total tendon load per synced day.
...
- Bar click selects the day or week. In Exposure, Load, and Work, the selected day/week appears as a persistent Garmin-style frosted callout anchored above the selected bar, with only the selected date, value, and metric label. Detailed numbers belong in the right panel, not in the floating callout. The callout is positioned outside the scrollable plot so edge selections are not clipped by the plot. It stays until another bar is selected, the chart context changes, or the dashboard closes.
...
- Load right panel shows the selected day/week, with the panel itself acting as the card. It must show `Total tendon load`, `Timed work total`, and fixed-order load sources: `Other computer`, `Computer work`, `Physical work estimate`. Weekly Load values are average per synced day.
- Work right panel shows the selected day/week, with the panel itself acting as the card. It must show `Work`, `Timed work total`, and only the work split: `Computer work` and `Physical work estimate`. It must not show `Other computer`. Weekly Work values are average per synced day.
