...
- View tabs sit under the modal titlebar and are `Exposure`, `Load`, `Work`, and `Breakdown`, each with a simple line icon plus text label.
...
- `Stack by: Categories / Groups` appears only in Breakdown. Load and Work have no load-basis toggle. Daily chart grain always shows the 7-day average line in Exposure, Load, Work, and Breakdown with a compact non-interactive legend centered in the chart footer below the x-axis. Weekly grain hides the 7-day average line and average legend. Methodology-change markers use the same chart-footer legend area when marked dates are visible.
- Right panel owns range analytics for Exposure, selected day/week analytics for Load and Work, and selected/range category analytics for Breakdown.
- The header wrench opens an overlaid resync drawer so charts do not move. Resync controls are the primary content; connection/debug metadata such as desktop, ActivityWatch version, server URL, bucket IDs, cached days, day start, and last sync stays collapsed under `Connection details` until requested. Advanced resync uses exact date strings through `maybeSyncActivityWatchDateStrings(...)`.

Chart interaction:

- Default view is Exposure with Daily grain.
- Exposure answers only how much active computer use happened. Daily Exposure uses one total Computer active time bar so the default view stays glanceable; Work and Breakdown own composition detail. Weekly Exposure groups Monday-Sunday and shows average computer active time per synced day.
- Load answers how much total tendon load happened. Daily Load stacks `Other computer`, `Computer work`, and `Physical work estimate` so the total and its sources are visible together. Weekly Load groups Monday-Sunday and shows the same sources as average per synced day.
- Work answers how much intentional/timed work happened. Daily Work splits only `Computer work` and `Physical work estimate`; it never includes `Other computer`. Weekly Work groups Monday-Sunday and shows average work per synced day.
...
- Load is `Computer active time + Physical work estimate`. Work is `Computer work + Physical work estimate`. The physical work estimate subtracts only `Computer work` from `Timed work total` so computer Work is not double-counted.
...
- Daily 7-day average is fixed at 7 days for v1. Each point is aligned to that day's bar and averages that day plus the previous 6 local waking-day dates. Exposure averages Computer active time. Load averages Total tendon load. Work averages Work. Breakdown averages the currently plotted metric: total Computer active time when unfiltered, or the locked category/group when filtered. Hover previews must not change the average line. Tooltip text must state the date, metric, and average value.
...
- Bar click selects the day or week and updates the right panel. Do not add a persistent floating selected-bar card: it duplicates the side panel and obscures trend comparison. Segment hover uses a small transient tooltip, and the selected bar remains visibly marked.
...
- Load right panel shows one selected day/week summary without nesting the total in another card. It must show the total as the heading value, a concise formula, `Timed work total` as a reference row, one composition bar, and fixed-order load sources: `Other computer`, `Computer work`, `Physical work estimate`. Weekly Load values are average per synced day.
- Work right panel uses the same flat hierarchy for `Work`, `Timed work total`, and only the work split: `Computer work` and `Physical work estimate`. It must not show `Other computer`. Weekly Work values are average per synced day.
...
- Methodology-change markers appear as subtle cyan/blue dashed chart markers behind the stacked bars in dashboard views, distinct from the yellow 7-day average line. The chart layer order should keep grid lines lowest, methodology markers below bar stacks, bar stacks below the 7-day average line, and the selected-bar arrow above the 7-day average line. Bar stacks need an opaque base paint layer so methodology markers do not bleed through semi-transparent Exposure, Load, or Work segments. When the visible dashboard range includes a marked date, show `Methodology change` in the chart-footer legend below the x-axis with a matching dashed vertical marker instead of inside the range controls, heading, or right detail panel. Hover/focus text should show the date, `Methodology change`, `Break in series`, and the detail text. Timeline day headers should show a compact marker chip, and copied timeline Markdown should include the same note for copied days.
